...
const processAssistantResponse = (response, toolResponses) => {
  ...
  const imageToolResponses = Array.isArray(toolResponses)
    ? toolResponses.filter((tr) => {
        try {
  ...
  if (imageToolResponses && imageToolResponses.length > 0) {
    // You can handle multiple images if needed
    const imageUrls = imageToolResponses
      .map((tr) => {
        try {
...
function ChatApp() {
  ...
  // Load API settings from environment variables first, then localStorage with security checks
  const [apiSettings, setApiSettings] = useState(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        if (/^[\[\{].*[\}\]]$/.test(saved)) {
          ...
          console.error("Invalid settings format in localStorage");
          localStorage.removeItem("apiSettings"); // Remove invalid data
        }
      }
    } catch (error) {
    ...
    return {
      ...
      tavilyApiKey:
        import.meta.env.VITE_TAVILY_API_KEY || savedSettings.tavilyApiKey || "",
      endpoint:
        import.meta.env.VITE_API_ENDPOINT ||
        savedSettings.endpoint ||
      ...
      streamingEnabled:
        savedSettings.streamingEnabled !== undefined
          ? savedSettings.streamingEnabled
          : true,
    };
  });
  ...
  const [systemPrompt, setSystemPrompt] = useState(
    "You are a helpful assistant with web search and image generation capabilities.",
  );
  ...
  // Save data to localStorage whenever it changes with security measures
  useEffect(() => {
    try {
      // Don't store sensitive API keys in localStorage if they're from env vars
      const secureApiSettings = { ...apiSettings };
      if (
        import.meta.env.VITE_OPENAI_API_KEY &&
        secureApiSettings.apiKey === import.meta.env.VITE_OPENAI_API_KEY
      ) {
        secureApiSettings.apiKey = "[ENV]"; // Don't store the actual key
      }
      if (
        import.meta.env.VITE_TAVILY_API_KEY &&
        secureApiSettings.tavilyApiKey === import.meta.env.VITE_TAVILY_API_KEY
      ) {
        secureApiSettings.tavilyApiKey = "[ENV]";
      }

      localStorage.setItem("conversations", JSON.stringify(conversations));
      localStorage.setItem("currentConversationId", currentConversationId);
      localStorage.setItem("apiSettings", JSON.stringify(secureApiSettings));
            ...
            conv.id === currentConversationId
              ? {
                  ...
                  messages: conv.messages.filter(
                    (msg) => !msg.content.startsWith("Uploading"),
                  ),
                }
              : conv,
          ),
        ...
        if (response.requiresToolCalls && response.message.tool_calls) {
          ...
          for (const toolCall of response.message.tool_calls) {
            ...
            if (functionName === "performWebSearch") {
              ...
              toolResult = await performWebSearch(
                query,
          ...
          setConversations((prev) =>
            prev.map((conv) => {
              ...
              if (messageIndex !== -1) {
                const pollinationsRegex =
                  /https:\/\/pollinations\.ai\/prompt\/([^)\s]+)/g;
    ...
    const summaryMessages = [
      {
        role: "system",
        content:
          "Summarize the following conversation into a short title (max 5 words):",
      },
